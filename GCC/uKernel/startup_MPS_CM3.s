...
UsageFault_Handler\
				...
				IMPORT MainSVCHandler
				IMPORT start_first_task
SVC_Handler     PROC
                EXPORT  SVC_Handler               [WEAK]
				; go to SP[6][-2] and there you'll find the argument of svc
				; then according to that branch to the right handler
				; let's give it a try
				tst lr, #4   ;;what stack were we using on syscall?
				mrseq r0, msp ; set that one as argument to MainSVCHandler
				mrsne r0, psp ;
				b MainSVCHandler ; handles the call
				bx lr
				ENDP
DebugMon_Handler\
                PROC
