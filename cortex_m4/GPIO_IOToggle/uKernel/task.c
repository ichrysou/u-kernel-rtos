...
static void task_lTCBInit(TCB *newTCB, uint_8 prio);
...
void task_initTasks(void) {
    ...
    for (i = 0; i < MAX_TASKS; i++) TaskArray[i] = NULL;

    for (i = 0; i < MAX_TASKS; i++) ReadyArray[i] = NULL;

#if HIGHEST_PRIO_ALT == 2

    for (i = 0; i < (MAX_TASKS >> 5); i++) ReadyTaskBitmap[i] = 0;
    ReadyTaskIndex = 0;
...
// it seems more convenient to do it like this:
...
void task_findHighestPriorityTask() { highestTCB = ReadyArray[31 - clz(ReadyTaskBitmap[31 - clz(ReadyTaskIndex)])]; }
/* Better alternative? */
#endif
...
err_t task_prioDisable(uint_16 prio) {
    if (TaskArray[prio] == NULL) return ERR_INVALID_PRIO;
