...
err_t queueSendToTail(queue *q, void *msg)
{
	TCB *tmp;
        ...
        memcpy((void *)q->tail, msg, q->elementSize);
...
//TODO: WARNING, watchit here, buff must be allocated to have the
//		same type (size) as the q->elementSize that is using it, otherwise problems
...
static void queueGetMessage(void *buff, queue *q)
{
	memcpy(buff, q->head, q->elementSize);
	q->length--;
